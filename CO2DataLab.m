...
A_lon_diff = abs(longrid - A_lon);
A_lon_index = find(A_lon_diff == min(A_lon_diff));
longrid(A_lon_index);

A_SST = squeeze(SST(R_lat_index, A_lon_index, :));
A_PCO2 = squeeze(PCO2_SW(R_lat_index, A_lon_index, :));
A_PCO2_T = squeeze(PCO2_T(R_lat_index, A_lon_index, :));
R_PCO2_BP = squeeze(PCO2_BP(R_lat_index, A_lon_index, :));


figure (3); clf
subplot(2,2,1)
plot(monthgrid, A_SST)
...
A_lat_diff = abs(latgrid - A_lat);
A_lat_index = find(A_lat_diff == min(A_lat_diff));
latgrid(A_lat_index);
...
A_SST = squeeze(SST(A_lat_index, A_lon_index, :));
A_PCO2 = squeeze(PCO2_SW(A_lat_index, A_lon_index, :));
A_PCO2_T = squeeze(PCO2_T(A_lat_index, A_lon_index, :));
A_PCO2_BP = squeeze(PCO2_BP(A_lat_index, A_lon_index, :));


figure (4); clf
subplot(2,2,1)
plot(monthgrid, A_SST)
title('Sea Surface Temperature at Station ALOHA');
xlabel('Months')
ylabel('Temperature (C)')
